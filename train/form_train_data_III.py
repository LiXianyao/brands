...
from similarity.brand import getCharacteristics, glyphApproximation
import csv
import time
import json
import  traceback
...
def form_train_data_redis(lowb, num):
    ...
    csv_name = u"train_7_2_4.csv"
    title = []
    if os.path.exists(csv_name) == False:
        ###文件不存在，则把表头写一下
        title = [u"输入商标名",u"历史商标名",u"输入商标编号",u"历史商标编号",u"所属大类",u"两个商标有共同小项？",u"汉字编辑距离相似度",u"拼音相似度",u"汉字包含被包含",
    ...
    record_time_dict = getHistoryBrandWithTime(record_key_time, db)
                        ...
                        try:
                            ...
                            for class_no in class_no_set:
                                ...
                                for i in range(len(compare_list)):
                                    ...
                                    last_class[True] = compare_unit
                                    if judge_pinyin(brand_name, his_name) == False:
                                        #if glyphApproximation(brand_name, his_name) < 0.7:
                                        continue
                                    #计算相似度
...
def judge_pinyin(brand_name, his_name):
    ...
    if len(b_list) < 3 and cnt_comm == len(b_list) and maxl <= len(b_list) * 3:
        #print b_list, h_list, cnt_comm
        return True
    elif len(brand_name) >= 3 and cnt_comm >= int(maxl * 0.65):#
        #print b_list,h_list
        return True
    return False
...
def getHistoryBrandWithTime(record_key_time, db):
    record_key_time_dict = {}
    for class_no in range(1, 46):
        ##依次获取每个大类的
        record_key_time_set = db.smembers(record_key_time + str(class_no))
        for record_key in record_key_time_set:
            ...
            try:
                ...
                record_key_time_dict[apply_time][class_no] = []
            record_key_time_dict[apply_time][class_no].append({
                                                        "name":brand_name,
                                                        "no":brand_no,
                                                        "status":brand_status
                                                    })
    return record_key_time_dict
