...
def test_data(taskId, task_dict):
    row_len = 18
    csv_name = task_dict["test_set"]
    input_test_file_name = taskId + "_test"
    upb = 10
    name_pair = set()
    with open(csv_name, "rU") as csv_file:
        ...
        for line in reader:
            ...
            input_line += "\n"
            if his_status == "1":
                input_lines.append((input_line,"&*(".join([this_name, his_name, class_no]) ) )

    save_input_file(input_lines, "data/input/" + input_test_file_name)
    test_model_name  = task_dict["test_model"]
    from classify_xgboost_train import test_model
    test_model(taskId, test_model_name)
...
if __name__ == "__main__":
    ...
    elif task_dict["task_target"] == "test":
        test_data(nowtime, task_dict)
    # load_brand_item()