...
def train_Data(taskId, task_dict, boost_parameters_dict, train_parameters_dict):
    ...
    with open(csv_name, "rU") as csv_file:
        ...
        for line in reader:
            ...
            #    continue
            product_no = line[-13]
            this_brand_no = line[-14]
            his_brand_no = line[-15]
            this_name = line[0]
            his_name = ",".join(line[1:-15])
            ...
            input_line += "\n"
            if his_status == "1":
                input_lines[this_status].append((input_line,"&*(".join([this_name, his_name, product_no]) ) )
    ...
    train_size = int(ratio * min(len(input_lines["0"]), len(input_lines["1"])))
    ...
    test_lines = input_lines["0"][train_size:]
    test_lines.extend(input_lines["1"][train_size:])
